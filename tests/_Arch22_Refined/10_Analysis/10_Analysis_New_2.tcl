...
set maxNumIter 300;                # Convergence Test: maximum number of iterations that will be performed before "failure to converge" is returned
...
set Nsteps [expr int($Fmax)];        # number of pushover analysis steps
set Nsteps_var	1
set Nsteps_var_demo	1

# set ok [analyze $Nsteps]; 


while {$Nsteps_var < $Nsteps} {


set ok [analyze $Nsteps_var]; 
	...
	puts "Step $Nsteps_var_demo done"
	set Nsteps_var_demo	[expr $Nsteps_var_demo+1]
	

	} elseif  {$ok != 0} {
	
	
		test $TestType 5.e-03 $maxNumIter $printFlag;	
		set ok [analyze $Nsteps_var];
		...
		if {$ok == 0} {
			...
			test $TestType 1.e-02 $maxNumIter $printFlag; 
			set ok [analyze $Nsteps_var];
			...
			if {$ok == 0} {
				...
				test $TestType 1.e-01 $maxNumIter $printFlag;
				set ok [analyze $Nsteps_var];
